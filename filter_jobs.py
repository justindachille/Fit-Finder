...
def filter_jobs():
    ...
    for job in tqdm(job_listings):
        if not job['sponsorship_checked']:
            ...
            prompt = f"Job Title: {job['title']}\nCompany: {job['company']}\nLocation: {job['location']}\nDescription: {job['description']}\n\nDoes this job offer sponsorship or not mention sponsorship? Answer in one word: YES if the job offers sponsorship or doesn't mention sponsorship. Answer NO if the job explicitly states that sponsorship is not offered or they are only hiring candidates who are already legally allowed to work in the country. Remember to answer in one word: yes or no."
